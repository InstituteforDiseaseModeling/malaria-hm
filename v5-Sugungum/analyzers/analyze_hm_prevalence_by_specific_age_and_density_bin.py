...
class Summary_Prevalence_by_Specific_Age_and_Density_Bin_Analyzer(BaseAnalyzer):

    def __init__(self, age_bins, density_bins, age_bin_query, density_bin_query, **kwargs):
        super().__init__(filenames=['output/MalariaSummaryReport_Monthly_Report.json'])
        self.age_bin_query = age_bin_query
        self.density_bin_query = density_bin_query
        self.metadata =  {
        'density_bins': density_bins,  # (, 0] (0, 50] ... (50000, ]
        'density_bin_edges':['0', '50', '200', '500'],
        'age_bins': age_bins,  # (, 5] (5, 15] (15, ],
    ...
    def finalize(self, all_data):
        ...
        for sim, data in all_data.items():
            ...
            for season in range(len(seasons)):
                ...
                age_bin_column = self.metadata['age_bins'].index(self.age_bin_query)
                density_bin_row = float(self.density_bin_query)
                specific_pfpr = season_pfprs.loc[density_bin_row, age_bin_column]
                pfpr_list.append(specific_pfpr)
        ...
        #define exp_id
        #how do I make this iter generalizable???
        results.to_csv(os.path.join('..', 'iter1', f'{exp_id}','analyzer_results.csv'))
...
if __name__ == '__main__':
    age_bins = [0, 1, 4, 8, 18, 28, 43, np.inf]
    density_bins = [0, 50, 200, 500, np.inf]
    am = AnalyzeManager('d8e821ca-c300-ea11-a2c3-c4346bcb1551',analyzers=Summary_Prevalence_by_Specific_Age_and_Density_Bin_Analyzer(age_bins = age_bins, density_bins = density_bins, age_bin_query = 1, density_bin_query = 200))
    am.analyze()
