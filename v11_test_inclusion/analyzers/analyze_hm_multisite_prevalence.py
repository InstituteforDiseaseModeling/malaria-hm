...
class Summary_Prevalence_Analyzer(BaseAnalyzer):
    ...
    def __init__(self, cut_age = 1, cut_density = 0, cut_season = 'W2',iteration = 0,**kwargs):
        ...
        self.cut_age = cut_age
        self.cut_density = cut_density
        self.cut_season = cut_season
    ...
    def finalize(self, all_data):
        ...
        channels = self.reference.index.levels[0].values
        seasons = [self.cut_season]
        age_bins = [self.cut_age]
        density_bins =[self.cut_density]
        ...
        for sim, data in all_data.items():
            ...
            for season in range(len(seasons)):
                for density_bin in density_bins:#change back to all density_bins for full report
                    for age_bin in range(len(age_bins)):
                        if density_bin == 0:
                            continue
                        elif density_bin == 50:
                            sim_value = data.loc[density_bin, seasons[season]][age_bin]+data.loc[0, seasons[season]][age_bin]

                        elif density_bin == 200:
                            continue
                        elif density_bin == 500:
                            sim_value = data.loc[density_bin, seasons[season]][age_bin] + data.loc[200, seasons[season]][
                                age_bin]
                        else:
                            sim_value = data.loc[density_bin, seasons[season]][age_bin]

                        # bin_value = data.loc[density_bin, seasons[season]][age_bin]
                        # all_pfprs.append(bin_value)
                        # sim_value = np.mean(all_pfprs)
                        sim_id = sim.id
                        sim.tags.get("__sample_index__")
                        sample = sim.tags.get("__sample_index__")
                        # use sub_results block below for full reporting
        ...
        site = self.metadata['village']
        iteration = self.iteration
        #define cut name
        cut_name = f'{self.cut_density}_{self.cut_age}_{self.cut_season}_{site}'

        #check if dir exists for this cut in this experiment
        if not os.path.exists(os.path.join('..', f'iter{iteration}', f'{exp_id}',cut_name)):
            os.makedirs(os.path.join('..', f'iter{iteration}', f'{exp_id}',cut_name))

        #save results to csv
        results.to_csv(os.path.join('..', f'iter{iteration}', f'{exp_id}',cut_name,'analyzer_results.csv'))
...
if __name__ == '__main__':
    cut_age = 1 #[1, 4, 8, 18, 28, 43, 125]
    cut_density = 50 #[50, 500, 2000000]
    cut_season = 'W2' #['DC2', 'DH2', 'W2'],
    iteration = 0
    am = AnalyzeManager('cf29731c-595e-ea11-a2c5-c4346bcb1550',analyzers=Summary_Prevalence_Analyzer(cut_age = cut_age, cut_density = cut_density, cut_season = cut_season,iteration = iteration))
    am.analyze()