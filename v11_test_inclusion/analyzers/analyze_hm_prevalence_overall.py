...
class Summary_Prevalence_Analyzer(BaseAnalyzer):
    ...
    def finalize(self, all_data):
        ...
        #
        #     bin_subset = grouping.iloc[1:, :]
        #     bin_subset.iloc[0, 6] += grouping.iloc[0, 6]
        #     bin_subset.iloc[0, 4] += grouping.iloc[0, 4]
        #     new_ref_df = pd.concat([new_ref_df,bin_subset])
        #
        # mean = np.nanmean([sum(x.proportion) for i, x in new_ref_df.groupby(by=['Season', 'Age Bin'])])
        ...
        for sim, data in all_data.items():
            ...
            for season in range(len(seasons)):
                for density_bin in density_bins[2:]:#change back to all density_bins for full report
